...
Python 로그 생성기 ┤
 ...
 factory-json-topic      factory-text-topic   <- 토픽 : 카프카에서 메세지 구분하는 용도
       ...
       Kafka UI(대시보드)        Vector (서비스) -> kafka 메시지 firehose 전송
                                  │
                                  ▼
                           AWS Firehose -> 버퍼링 -> s3(브론즈 혹은 오픈서치 전송 실패한 데이터만 collect) or opensearch(인덱스 (센서/공장관리단위) 검색) -> Airflow DAG 활용 -> 메달리온 아킥텍처 적용
```
# 디렉토리 구조
...
│                          └─windows_amd64
│                                  LICENSE.txt
│                                  terraform-provider-aws_v6.62.0_x5.exe
│                                  
├─logstash.conf
└─sensor_logs
        sensor_json.log
...
# Vector
- Datadog사에서 개발한 도구
- Rust 개발, 로그, 매트릭 수집, 변환, 전송처리, 초고성능, 경량 파이프라인 도구
- 로그 확인 
    - 
    ```
...
https://search-de-ai-22-kakfa-efk-os-36lnvgun6zis4p6fombw3ef6ay.ap-northeast-2.es.amazonaws.com/factory-sensor-001/_count
